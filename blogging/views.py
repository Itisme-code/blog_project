...
def blogs(request,category=None):
    ...
    if title:
        blogs = blogs.filter(title__icontatins = title)
...
def like_view(request,blog_id):
    ...
    return JsonResponse({"like":blog.likes.count(),"dislike":blog.dislikes.count()})
...
def subscribe_view(request,profile_id):
    profile = Profile.objects.get(id=profile_id)
    if hasattr(request.user,"profile"):
        profile.add_subscribe(request.user.profile)
        print(profile.subscribers.count())
    return JsonResponse({"subscribe":profile.subscribers.count()})
